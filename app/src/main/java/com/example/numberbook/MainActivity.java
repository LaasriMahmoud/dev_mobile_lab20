...
public class MainActivity extends AppCompatActivity {
    ...
    private void syncContactsToServer() {
        if (contactList.isEmpty()) {
            Toast.makeText(this, "Aucun contact a synchroniser. Chargez les d'abord.", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        for (Contact contact : contactList) {
            contactApi.insertContact(contact).enqueue(new Callback<ApiResponse>() {
                ...
                public void onFailure(@NonNull Call<ApiResponse> call, @NonNull Throwable t) {
                    Toast.makeText(MainActivity.this, "Erreur reseau", Toast.LENGTH_SHORT).show();
                }
            });
        }
